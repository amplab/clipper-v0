            ...
            if is_fixed_format(input_type):
                ...
                for i in range(0, num_inputs):
                    ...
                    if input_type == VARBYTE_CODE:
                        ...
                        # Our inputs are 8-byte floats, so we must read 8 * input_len 
                        # bytes from the packed data
                        input_len = input_len * 8
                        inputs.append(np.array(array.array('d', bytes(data[:input_len]))))
                    else:
                        assert input_type == VARINT_CODE
                        # Our inputs are 4-byte integers, so we must read 4 * input_len
                        # bytes from the packed data
