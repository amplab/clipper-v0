          ...
          S: Serialize + Deserialize
{
    ...
    cache: SimplePredictionCache<Output>,
}
          ...
          S: Serialize + Deserialize
{
    pub fn new(conf: ClipperConf) -> ClipperServer<P, S> {
        ...
        // Connect to model wrappers and create batching schedulers

        // Create shared prediction cache
...
impl<P, S> PredictionWorker<P, S>
    where P: CorrectionPolicy<S>,
          S: Serialize + Deserialize
{
    pub fn new(worker_id: i32) -> PredictionWorker<P, S> {
        let (sender, receiver) = mpsc::channel::<PredictionRequest>();
        thread::spawn(move || {
            PredictionWorker::run(worker_id, request_queue);
        });
        PredictionWorker {
            worker_id: worker_id,
            input_queue: sender,
